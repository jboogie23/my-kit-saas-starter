...
import { type SettingsNotificationsFormSchema, settingsNotificationsFormSchema } from "$validations/app/settings";
...
export const load: PageServerLoad = async ({ locals, cookies, url }) => {
  ...
  const form = await superValidate<SettingsNotificationsFormSchema>({ name: user.name }, zod(settingsNotificationsFormSchema));
...
export const actions: Actions = {
  default: async (event) => {
    ...
    const form = await superValidate<SettingsNotificationsFormSchema>(request, zod(settingsNotificationsFormSchema));
    if (!form.valid) {
